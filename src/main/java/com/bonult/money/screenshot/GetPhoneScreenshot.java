...
public class GetPhoneScreenshot implements GetScreenshot {
	...
	public File getImgFile(){
		...
		try{
			Process process = Runtime.getRuntime().exec(ConfigHolder.CONFIG.getAdbPath() + " shell /system/bin/screencap -p /sdcard/1pic/screenshot.png");
			process.waitFor();
			if(process.exitValue() == 0){
				process = Runtime.getRuntime().exec(ConfigHolder.CONFIG.getAdbPath() + " pull /sdcard/1pic/screenshot.png " + imgFileName);
				process.waitFor();
				if(process.exitValue() == 0){
